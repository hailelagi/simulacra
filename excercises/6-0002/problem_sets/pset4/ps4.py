...
def generate_models(x, y, degs):
    ...
    """
    return [np.array(np.polyfit(x, y, n)) for n in degs]
...
def r_squared(y, estimated):
    ...
    for i in range(limit):
        s_s_res.append((y[i] - estimated[i]) ** 2)
        s_s_tot.append((y[i] - mean) ** 2)

    return 1 - sum(s_s_res)/sum(s_s_tot)
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        pylab.plot(x, est_y, 'r', label="degree of your regression model" +
                   "R - Squared is" + str(round(error, 5)))
